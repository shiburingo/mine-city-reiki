...
50万語未満では加速収集モードを自動選択し、1バッチ予算をWikipedia 100,000件、Wiktionary 50,000件、Wikidata 100語に増やします。日次ジョブは初回構築中のみ最大4バッチを連続実行し、50万語到達、語数増加停止、設定上限のいずれかで終了します。50万語到達後は通常収集（5,000件 / 2,000件 / 25語）を1バッチだけ実行します。MediaWikiは500件のリダイレクト元と転送先を1リクエストで解決し、単一並列・毎秒4リクエスト以下・`maxlag`付きで巡回します。

取得効率は文字種や重複率によって変動します。2026年7月23日の本番実測では、Wikipedia 100,000件を約8分で巡回し、コンパイル済み辞書が106,712語から210,284語へ103,572語増加しました。この実績では初回50万語を1回の日次実行枠内（概ね30〜40分）で構築できる見込みです。途中で429/503が返った場合は既存カーソルを維持し、次回実行で継続します。
