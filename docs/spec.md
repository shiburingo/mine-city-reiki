...
- 会議録検索だけをログイン不要の公開ページとして提供し、管理機能を露出せずに他部署・議員・市民が利用できるようにする。
...
- 公開URLは `/mine-city-minutes/` とし、Viteの独立HTMLエントリから既存の会議録ワークスペースを起動する。
- UI実装、MariaDB、会議録コンパイル世代、Meilisearch索引、関連語辞書は管理画面と共用し、公開用の別DBや別索引は作らない。
- 公開モードでは認証API、同期履歴、辞書状態、同期・再タグ付け・コンパイルなどの管理APIを呼び出さない。
- 公開検索履歴は管理画面と異なるlocalStorageキーへ保存し、同じブラウザでも履歴を混在させない。
- 会議録PDFの内容確認が必要な場合は、美祢市が公開するPDF原文へ誘導する。
...
| 公開会議録ページ | ログイン不要の会議録閲覧・言葉検索・発言者検索・発言集作成。管理タブと管理操作は表示しない |
...
- 認証不要の例外は `/api/public/minutes/status`、`search`、`meetings`、`speakers`、`meetings/:id`、`days/:id` の GET / HEAD に限定する。
- 公開APIはIP単位のレート制限、クエリ文字数・クエリ文字列全長の上限を適用し、公開ステータスから内部実行ID・エラー詳細・コンパイル情報を除外する。
- `/api/minutes/*` と、公開プレフィックス下の未許可パス・変更系HTTPメソッドは従来どおり認証を必須とする。
