...
- 新規関連語ペアは語順を正規化して保存し、バイナリ照合の生成列による無向ペア一意索引で `A → B` と `B → A` の逆向き重複を防ぐ。既存重複は手動登録と高優先度を保持して統合し、ひらがな・カタカナ・濁点などの表記差は別語として維持する。
...
初期版では直近1年分のPDFを対象に、DB上の発言単位・表単位データを検索する。会議録専用Meilisearch投入は次段階の高速化対象とし、APIとUIは専用インデックスへ差し替えられるよう会議録系テーブルとエンジンを分離している。
