...
- 100万語は上限ではない。以後も件数より品質を優先する。`dictionary_policy.py` に会議録検索の展開上限5語、表記揺れ判定、自治体用語の優先順位を集約する。
- 設定の「辞書の増強」から自動収集と手動取り込みを中断/再開できる。`dictionary_growth_settings` の単一行に状態と変更世代を保存する。初期状態は許可。起動時の初期化で既存の中断設定を上書きしない。
- 中断時は次の通信ページ/出典/会議録バッチの安全な区切りで停止し、未完了の取込トランザクションは取り消す。保存済みデータ・カーソルは保持して再コンパイルする。停止直後に再開されても旧ジョブは変更世代の差を検知して終了する。再開は次回定期実行または手動起動から適用する。
- 増強停止は検索・既存辞書の再コンパイル・個別の手動訂正を妨げない。中断履歴は `sync_runs.summary_json.outcome=paused`（DB上は正常終了）とし、画面には「中断」と表示する。設定APIは公開APIに含めず、ゲストは変更不可。軽量な登録語数取得APIにはDB設定読み出しを追加しない。
...
- 管理済み同義語、Wiktionary、選別したWikipediaの順に収集する。Wikipediaは明示的な表記揺れまたは自治体用語を含む別名だけを新規採用し、記事内節への転送は両ソースとも除外する。未採用件数はソース統計の `deferredByPolicy` に記録する。既存の収集済みペアは一括削除しない。
- Wikidataは手動・管理済み・既存DB・会議録由来の語を `priorityLastSynonymId` で順番に補強する。旧一般語カーソルと分けて、優先対象を初回から巡回する。肩書きと人物、同じ話題の語などの抽出ペア自体を同義語とは扱わない。
- 出典付きの候補は手動設定・管理済み言い換え・表記揺れ・自治体の別名を優先して検索展開する。出典フィルタと最低優先度8、1段階のみの展開、入力語ごとに最大5語という制限を維持する。低信頼ペアは自治体の語を含んでも昇格させない。
- 旧管理済みシードの「上下水道と水道」「養鱒場と養魚場」のような広義・狭義の関係は保存したまま、会議録の同義語展開から除く。手動で明示的に承認したペアは引き続き利用できる。
...
会議録は公開済みの平成20年以降を取り込み、有効世代の本文全体をSQLite FTS5の1文字・2文字索引へコンパイルする。通常・無制限とも同じ検索条件を使い、関連語は原語ごとのORグループをAND/ORで結合する。検索結果は抜粋だけを返し、全文は展開時に取得する。続きはスコア・検索条件識別子付きカーソルで追加取得する。索引が利用できない場合は `meeting_compiled_utterances` のMariaDB文字列照合へフォールバックする。詳細は `meeting-minutes-search-design.md` を参照。

### 公開ページ

- 公開URLは `/mine-city-minutes/` とし、Viteの独立HTMLエントリから既存の会議録ワークスペースを起動する。
- UI実装、MariaDB、会議録コンパイル世代、SQLite文字索引、関連語辞書は管理画面と共用し、公開用の別DBや別索引は作らない。
- 公開モードでは認証API、同期履歴、辞書状態、同期・再タグ付け・コンパイルなどの管理APIを呼び出さない。
- 公開検索履歴は管理画面と異なるlocalStorageキーへ保存し、同じブラウザでも履歴を混在させない。
- 公開ページの表示モードとカラーパレットも管理画面とは異なるlocalStorageキーへ保存する。初回表示はライトモードと「別府弁天池」を既定値とし、ヘッダーの設定から変更できる。
- 管理画面のタブ列には「会議録検索システム（公開）」を表示し、管理中の画面を保持したまま公開ページを新しいタブで開けるようにする。
...
- 発言者候補は業務システム版・公開版で共通の分類処理を使用し、「議員」「執行部」「その他」をトグルで切り替えて表示する。議会事務局は「その他」、議長・委員長・仮議席は「議員」として扱う。
- 同日会議録の高信頼な出席者名簿と、氏名・役割・肩書きが安全に照合できる場合は、PDF抽出時の氏名1文字誤りを正規名へ補正する。年度辞書だけを根拠にした曖昧な名寄せは行わない。
