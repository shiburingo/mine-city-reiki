...
- **累積シソーラス** — Wikipedia・Wiktionary・Wikidata・WordNet・例規・会議録から出典付きで関連語を蓄積し、50万語を第1目標として日次更新します。50万語未満では最大4バッチを連続収集し、到達後は通常予算1バッチへ自動復帰します。検索時は100万語超を想定したSQLite索引と語単位LRUキャッシュを使用します。
...
- **本文内リンク** — 条番号、別表、元XMLアンカー、同一文書内参照、地方自治法・地方公務員法への文書間参照を解決します。文書間移動でも対象条文の先頭へ移動し、「リンク元に戻る」で元文書の参照位置を復元します。解決できない参照は誤誘導を避けるためリンク化しません。
